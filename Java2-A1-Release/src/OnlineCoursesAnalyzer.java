...
//        return null;
    ...
    public List<String> searchCourses(String courseSubject, double percentAudited, double totalCourseHours) {
        ...
        for(Course c : courses){
            ...
            if(matcher.find() && c.percentAudited >= percentAudited && c.totalHours <= totalCourseHours && !a.contains(c.title)){
                a.add(c.title);
            }
        }
        Collections.sort(a);
    ...
    public List<String> recommendCourses(int age, int gender, int isBachelorOrHigher) throws ParseException {
        ...
        Map<String, Double> PMale = new HashMap<>();
        Map<String, Double> PDegree = new HashMap<>();
        for(String key : a1.keySet()){
            ...
            PDegree.put(key, temp);
        }

//        for(String key : a1.keySet()){
//            System.out.println(key + ": " +a1.get(key) + ": " + AverageAge.get(key) +" "+ PMale.get(key) +" "+ PDegree.get(key));
//        }
        ...
        for(String ss : a3.keySet()){
            ...
            if((count1 = b.get(title)) == null){
                count1 = Math.pow(age - AverageAge.get(ss), 2) + Math.pow(gender * 100 - PMale.get(ss), 2) + Math.pow(isBachelorOrHigher * 100 - PDegree.get(ss), 2);
                b.put(title, count1);
            }else{
                Double temp = Math.pow(age - AverageAge.get(ss), 2) + Math.pow(gender * 100 - PMale.get(ss), 2) + Math.pow(isBachelorOrHigher * 100 - PDegree.get(ss), 2);
                if(temp < count1){
                    b.put(title, temp);
                }
            }
